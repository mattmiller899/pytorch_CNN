                        ...
                        for PAT in ${PATIENCES[@]}; do
                            for NO_N in ${NO_NS[@]}; do
                                for FLAG in "${FLAGS[@]}"; do
                                    ...
                                    OUT_FILE="${OUT_DIR}/${KMERSTR}mer_${FILTER}f_${CONV}nc_${FC}fc_${PAT}pa_${EPOCHS}eps_${OUTFLAGS}_1000filts.txt"
                                    ...
                                    init_dir "$FIG_DIR"
                                    export PY_ARGS="${FLAG} -gd ${GV_DIR} -vd ${V_DIR} -p ${NEWPOS_DIR} -s ${NEWSEQ_DIR} -a ${AA_DIR} -b ${BATCH} -e ${EPOCHS} -c ${NEWCONT_DIR} -o ${OUT_FILE} -f ${FIG_DIR} -fs ${FILTER} -nc ${CONV} -nf ${FC} -pa ${PAT} -kf ${KFOLD} ${KMER}"
                                    echo $PY_ARGS
                                    if [[ $FLAG == *-g* ]]; then
                                        echo "Using GPU"
                                        ...
                                        JOB_ID=`sbatch $ARGS --export=PY_ARGS --job-name=t${KMERSTR}_${FC}fc_${CONV}c_${FILTER}f_${READ} -e $STDERR_DIR/%x.err -o $STDOUT_DIR/%x.out ./runs/run_train_cnn_multikmer_gpu.sh`
                                    else
                                        echo "Using CPU"
                                        #JOB_ID=`qsub $ARGS -v PY_ARGS -N train_${KMERSTR} -e $STDERR_DIR -o $STDOUT_DIR ./run_train_cnn_multikmer_cpu.sh`
                                        JOB_ID=`sbatch $ARGS --export=PY_ARGS --job-name=t${KMERSTR}_${FC}fc_${CONV}c_${FILTER}f_${READ} -e $STDERR_DIR/%x.err -o $STDOUT_DIR/%x.out ./runs/run_train_cnn_multikmer_cpu.sh`
                                    fi
                                    if [ "${JOB_ID}x" != "x" ]; then
                                        echo Job: \"$JOB_ID\"
                                    else
