...
def main(args):
    ...
    # Load data
    all_reads, all_labels, all_freqs, kmer_dict, num_kmers_per_read = utils.read_fastas_from_dirs_CNN_freqs(
    ...
    for (tmp_train_idx, dev_idx) in dev_kf.split(dummy_arr):
        break
    dev_dataset = utils.ReadsDatasetCNNFreqs(all_reads[dev_idx], all_labels[dev_idx], all_freqs[dev_idx])
    ...
    train_test_reads, train_test_labels, train_test_freqs = all_reads[tmp_train_idx], all_labels[tmp_train_idx], all_freqs[tmp_train_idx]
    ...
    with open(output_file, "w") as out:
        ...
        for curr_fold, (train_idx, test_idx) in enumerate(kf.split(dummy_arr)):
            train_dataset = utils.ReadsDatasetCNNFreqs(train_test_reads[train_idx], train_test_labels[train_idx], train_test_freqs[train_idx])
            test_dataset = utils.ReadsDatasetCNNFreqs(train_test_reads[test_idx], train_test_labels[test_idx], train_test_freqs[test_idx])
            ...
            test_dataloader = torch.utils.data.DataLoader(test_dataset, batch_size=args.batch_size, shuffle=True, num_workers=0,
                                                          drop_last=False)
            #Create model
            model = mods.KmerCNNFreqs(all_embs,
                                num_kmers_per_read,
                                num_channels,
                                args.num_convs,
                                args.num_fcs,
            ...
            while curr_epoch < args.epochs:
                ...
                total_loss = 0.0
                running_loss = 0.0
                for batch_count, (data, labels, freqs) in enumerate(train_dataloader):
                ...
                if args.debug:
                    ...
                    utils.print_mem_usage(device)
                with torch.no_grad():
                    for data, labels, freqs in dev_dataloader:
                        data, labels, freqs = data.to(device), labels.float().to(device), freqs.float().to(device)
                        model.zero_grad()
                        opt.zero_grad()
                        if args.debug:
            ...
            # TODO finish testing
            test_loss, acc, f1, prec, rec = utils.test_CNNFreqs(model, test_dataloader, criterion, device, args, num_classes)
